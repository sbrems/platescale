        ...
        for i_src,xy_src in zip(sex_coords[i_im].index.values,sex_coords[i_im][['x_image_sex','y_image_sex']].as_matrix()):
            ...
            if len(idx_med) == 0:
                ...
                if len(idx_ign) ==1: 
                    ...
                    if verbose: print('No corresponding source found in median image. Maybe source %d (%d,%d)was split? Ignoring image nr %d.' %(i_src,y_src,y_src,i_im) )
                    ign_im.append(i_im)
                    break
            ...
            #now differentiate the different cases in the found sources:
                ...
                if len(multi_im) != len(idx_med):
                    if len(multi_im) == 1 and len(idx_med) == 2:
                        ...
                        if verbose: print('Only sources %s found in image nr %d, but %s in median image. Ignoring image.' %(multi_im,i_im,idx_med) )
                        ign_im.append(i_im)
                        break
...
def connect_sources(sex_coords,source_med,n_images=None,verbose=True):
    if verbose: print('Connecting and verifying matches of ', n_images,' images')
    excluded = pd.DataFrame()
    for aa in range(n_images):
        #set values to nan and then to the value
        for val in ['x_data_rot','y_data_rot',star_id,'offset','ra_now','dec_now']:
        ...
        for bb in range(len(sex_coords[aa]['x_image'])):
            for cc in range(len(source['x_data_rot'])):
                ...
                if dist <= search_rad:
                    if not np.isfinite(sex_coords[aa]['x_data_rot'][bb]): #new entry
        ...
        sex_coords[aa] = sex_coords[aa][np.isfinite(sex_coords[aa]['x_data_rot'])]
        ...
        multiples = sex_coords[aa][sex_coords[aa].duplicated(subset=('x_data_rot','y_data_rot'),keep=False)]
        ...
        #check if there are as many stars in catalogue around as duplicates 
            ...
            if (len(multiples) > len(i_stars_around)):
                ...
                sex_coords[aa] =sex_coords[aa].iloc[np.where(sex_coords[aa]['x_data_rot'] != multiples['x_data_rot'][0])]
                sex_coords[aa].reset_index(inplace=True,drop=True)
...
def connect_median_sources(source_cat,median_sources,verbose=True):
    '''Find the corresponding source to each source found in the median image 
        ...
        if i_med not in med2cat.keys(): #check this for the case of multiples
            ...
            if len(id_cat) == 0: #no source in cat found:
                ...
                if len(multi_med) < len(id_cat):
                    raise ValueError('There are multiple sources in the catalogue at the corresponding '+
                    'position in median position. Please check from hand which is the right one and '+
                    'remove the other from the catalogue.x/y med/cat:',median_sources.x_image[multi_med],
                                     median_sources.y_image[multi_med],
                                     source_cat.x_data_rot[id_cat],
                                     source_cat.y_data_rot[id_cat])
                elif len(multi_med) == len(id_cat): #same number of sources...minimize their distances
...
def match_smallest_dist(list1,list2):
    ...
    #now choose the minimum
    i_min_comb = np.where(comb_len == np.min(comb_len))[0]
