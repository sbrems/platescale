...
def coordinates(data_cube,header_cube,target,fn_sextr_sgl,fn_sextr_med,med=False,verbose=True):
    ...
    if not med:
        ...
        for kk in range(n_images):
            ...
            call(['sex','temp.fits','-c',fn_sextr_sgl,
                  '-checkimage_name',os.path.join(dir_temp,target+'_objects_{:03}.fits'.format(kk)),
        ...
        for kk in range(n_images):
            all_objects[kk,:,:] = fits.getdata(target+'_objects_{:03}.fits'.format(kk))
    ...
    if med:
        ...
        call(['sex','temp.fits','-c',fn_sextr_med,
              '-checkimage_name',os.path.join(dir_temp,target+'_objects_med.fits'),
              '-catalog_name',os.path.join(dir_temp,target+'_med.sex')],stdout=FNULL,
        ...
        all_objects = np.full(data_cube.shape, np.nan)
        sex_coords = [None]
        all_objects = fits.getdata(os.path.join(dir_temp,target+'_objects_med.fits'))
        sex_coords =pd.read_csv(os.path.join(dir_temp,target+'_med.sex'), header=None,
                                delimiter=r'\s+',comment='#',
